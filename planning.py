                ...
                for dx in range(-5, 6):
                    ...
                    if 0 <= nx < W and 0 <= ny < H and x0<=nx<x1 and y0<=ny<y1:
                        ...
                        if grid[ny][nx] >= self.free_th:
                            free_count += 1
            return total > 0 and free_count / total > 0.5 # 稍微降低阈值增加容忍度 2.json的迷宫要改成0.7
...
class Planner:
    ...
    def _gen_waypoint_explore(self, curr_pose_world: Pose) -> None:
        ...
        #self.current_waypoint = self._pixnode_to_local_waypoint(curr_pose_world, self.goal_path_pix[3])
