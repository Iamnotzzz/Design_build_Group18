            ...
            if frm.status == 0 and frm.cmd_id == self.last_cmd_id:
                ...
                if not self.in_flight:
                    ...
                    if self.counts==0:
                        ...
                        self.turn_deg, self.distance_m, self.report["report"] = command

                    self.last_frm_status = 1
                    """
                    if command is None:
                        print("任务已完成，停止控制线程。")
                        break
                    ...
                    if self.counts == 0:
                        self.link.send_cmd(self.last_cmd_id,turn_rad=self.turn_deg, distance_m=0)
                        self.counts+=1
                    elif self.counts == 1:
                        self.link.send_cmd(self.last_cmd_id, turn_rad=0, distance_m=self.distance_m)
                        self.counts = 0


                    self.in_flight = True
                    print("#"*50)
                    print(f"cmd_id{self.last_cmd_id},turn_deg{self.turn_deg},straight_dist_m{self.distance_m}")
                    print("#" * 50)
...
parser.add_argument("--cpr", type=int, default=1560, help="编码器CPR")
...
def main():
    ...
    GOAL_WORLD_COORDS = (2.3, 2.3)  # 例如：终点在起点右侧2.5米、前方1.8米
    planner = Planner(lgm,wheel_geom,goal_world=GOAL_WORLD_COORDS,vel_limits=VelocityLimits(),tol=CmdTolerance())
