...
class ListGridMap:
    ...
    def __init__(self, MAP_SIZE_PIXELS, MAP_SIZE_METERS):
        ...
        # 投票参数
        self.vote_params = {
            'obstacle_lock_threshold': 3,  # 障碍物锁定需要的投票数
            'free_lock_threshold': 4,  # 自由空间锁定需要的投票数
            'conflict_resolve_ratio': 2.0,  # 冲突解决比例：一方票数需要是另一方的几倍才能获胜
                    ...
                    if 0 <= ny < slam_map.shape[0] and 0 <= nx < slam_map.shape[1]:
                        ...
                        if self.is_locked[ny, nx]:
                            ...
                            if locked_val >= FREE_THRESH:
                                ...
                                obstacle_neighbor_count += 3  # 锁定的障碍物权重更高
                        else:
                            # 非锁定区域使用当前SLAM值
                            val = slam_map[ny, nx]
                            if val >= FREE_THRESH:
